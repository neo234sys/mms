...
public interface UnitReserveDetailsRepository extends JpaRepository<UnitReserveDetails, Integer> {
	...
	@Query("SELECT urd FROM UnitReserveDetails urd WHERE urd.reserveEndDate < CURRENT_TIMESTAMP AND urd.unit.unitStatus.unitStatusName = 'RESERVED' and urd.status='RESERVED'")
