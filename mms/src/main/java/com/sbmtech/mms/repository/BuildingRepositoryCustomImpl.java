...
public class BuildingRepositoryCustomImpl extends JdbcCommonDao implements BuildingRepositoryCustom {
	...
	public Page<BuildingNewDTO> searchWithSpecAndReturnDTO(Specification<Building> spec, Pageable pageable) {
		...
		List<Building> buildings = this.getEm().createQuery(query).setFirstResult((int) pageable.getOffset())
...
//		        Page<UnitNewDTO> unitPage = getFilteredUnitsForBuilding(building.getBuildingId(), unitFilter, unitPageable);
			...
			return new BuildingNewDTO(building.getBuildingId(), building.getBuildingName(), building.getAddress(),
	...
	public Page<UnitNewDTO> getFilteredUnitsForBuilding(Integer buildingId, UnitFilterNewRequest unitFilter,
			Pageable pageable) {
		CriteriaBuilder cb = this.getEm().getCriteriaBuilder();
...
//	    }
		...
		List<Unit> units = this.getEm().createQuery(unitQuery).setFirstResult((int) pageable.getOffset())
				.setMaxResults(pageable.getPageSize()).getResultList();

		// Convert units to DTOs
		List<UnitNewDTO> unitDTOs = units.stream()
				.map(unit -> new UnitNewDTO(unit.getUnitId(), unit.getUnitName(), unit.getFloor().getFloorName(),
						unit.getUnitStatus().getUnitStatusName(), unit.getUnitType().getUnitTypeName(),
